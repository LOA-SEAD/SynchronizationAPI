...
//	}
	...
	var botaoRanking = document.createElement("div");
	botaoRanking.setAttribute("id" , "btnRanking");
	botaoRanking.setAttribute("tabIndex" , "0");
	botaoRanking.setAttribute("role" , "button");
	botaoRanking.setAttribute("aria-label" , "Créditos");
	botaoRanking.setAttribute("class" , "botao");
	caixaBotoes.appendChild(botaoRanking);

	botaoRanking.onfocus = function() {
		adicionarComandosEnterSpace(ativarBotaoRanking, botaoRanking);
	}
	botaoRanking.onblur = function() {
		removerComandosEnterSpace();
	}
	botaoRanking.onclick = function()
	{
		ativarBotaoRanking();
	}
}
...
function criarCamadaRanking()
{
	...
	for (i = 0; i < ranking.length; i++) {
	    alert('jogador: ' + ranking[i]["jogador"]);
